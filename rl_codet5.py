...
dev_dataloader = DataLoader(train_data, batch_size=args.train_batch_size, shuffle=True)
...
for ep in range(args.train_epochs):
    ...
    for batch in pbar:
        ...
        for samp in range(n_syn_samples):
            ...
            reward1,mean_comp_rate,mean_ast_match,mean_dfg_match, num_errors,num_errors_ref, num_nodes,num_nodes_ref  = get_binary_compilation_reward(lang = 'python', code_ids=response_ids,code_ref_ids=response_ids_ref, gold_ids=target_ids, tokenizer=tokenizer)
            ...
            with open(direct_path + 'results/codet5_ppo_cpp_python6.csv', 'a') as f:
    ...
    path = args.output_dir + '/codet5_ppo' + '_reward%d'%(args.reward_id) +  '_bs%d'%(args.train_batch_size) + '_in-len%d'%(args.max_source_length) + '_out-len%d'%(args.max_target_length) +'_r%d'%(args.run)
    output_dir = os.path.join(path, 'checkpoints_0-1reward_r%d'%(args.run))
    ...
    nerrors_test, nerrors_ref_test = test(ep,test_features, test_dataloader, 'test')
